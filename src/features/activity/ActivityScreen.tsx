...
import { getDevices } from "@/lib/tauri/devices";
...
export function ActivityScreen() {
  ...
  useEffect(() => { getActivity().then(setEvents).catch(() => setError("Could not load activity history.")); getDevices().then(setInventory).catch(() => setInventory([])); }, []);
  const devices = useMemo(() => {
    const names = new Map(inventory.map((device) => [device.id, device.name]));
    return [...new Set((events ?? []).map((event) => event.deviceId).filter((id): id is string => Boolean(id)))].map((id) => ({ id, label: names.get(id) ?? "Deleted device" }));
  }, [events, inventory]);
