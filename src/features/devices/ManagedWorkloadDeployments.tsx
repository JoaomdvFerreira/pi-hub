...
function ManagedWorkloadCardView({ workload, maintenanceInProgress, onChange }: { workload: ManagedWorkloadCard; maintenanceInProgress: boolean; onChange: () => Promise<void> }) {
  const [prepared, setPrepared] = useState<PreparedManagedWorkload | null>(null); const [preparing, setPreparing] = useState(false); const [continuing, setContinuing] = useState(false); const [checkingStatus, setCheckingStatus] = useState(false); const [notice, setNotice] = useState<string | null>(null); const [observed, setObserved] = useState<ManagedWorkloadDeployment | undefined>(workload.deployment);
  useEffect(() => { setObserved(workload.deployment); }, [workload.deployment]);
  ...
  const checkStatus = async () => { if (checkingStatus || !deployment) return; setCheckingStatus(true); setNotice(null); try { setObserved(await reconcileManagedWorkloadDeployment(workload.workloadId)); await onChange(); } catch (error) { setNotice(`${apiMessage(error, "Pi-Hub could not check deployment status.")} The last known deployment state is still shown.`); } finally { setCheckingStatus(false); } };
