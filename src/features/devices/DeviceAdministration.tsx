...
export function DeviceAdministration({ deviceId, deviceName, maintenanceInProgress = false }: { deviceId: string; deviceName: string; maintenanceInProgress?: boolean }) {
  ...
  return <section className="rounded-lg border border-border bg-card p-3.5" aria-label="Device administration">
    ...
    {maintenanceInProgress ? <p className="mb-3 rounded-md border border-status-warning/40 bg-status-warning/10 p-2 text-xs text-foreground" role="status">Device maintenance is in progress.</p> : null}
    <div className="grid gap-2 sm:grid-cols-2">{ADMINISTRATION_ACTIONS.map(({ type, label, confirmation, danger, Icon }) => <AlertDialog key={type}><AlertDialogTrigger asChild><Button variant={danger ? "destructive" : "outline"} size="sm" className="justify-start" disabled={running !== null || maintenanceInProgress || expected?.operationType === "shutdownDevice"}><Icon className="size-3.5" />{running === type ? <Loader2 className="ml-auto size-3.5 animate-spin" /> : null}{label}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{label} “{deviceName}”?</AlertDialogTitle><AlertDialogDescription>{confirmation}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant={danger ? "destructive" : "default"} onClick={() => execute(type)}>{label}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>)}</div>
