...
pub struct BenchmarkConfig { pub name: String, pub scenario: String, pub sample_interval_ms: u64, pub max_duration_ms: u64, pub max_samples: usize }
impl Default for BenchmarkConfig { fn default() -> Self { Self { name: "Manual".into(), scenario: "manual".into(), sample_interval_ms: 1_000, max_duration_ms: 900_000, max_samples: 900 } } }
impl BenchmarkConfig { pub fn validate(&self) -> Result<(), String> { if self.name.trim().is_empty() || self.scenario.trim().is_empty() { return Err("Benchmark name and scenario are required.".into()) } if self.sample_interval_ms == 0 || self.max_duration_ms == 0 || self.max_samples == 0 { return Err("Benchmark bounds must be positive.".into()) } Ok(()) } }
...
#[derive(Clone, Debug, Serialize, Deserialize)] #[serde(rename_all="camelCase")]
pub struct SessionSummary { pub id: String, pub name: String, pub scenario: String, pub started_at_unix_ms: u128, pub ended_at_unix_ms: Option<u128>, pub duration_ms: u64 }
#[derive(Clone, Debug, Serialize, Deserialize)] #[serde(rename_all="camelCase")]
