...
fn process_cpu_percent(previous: ProcessCpuTiming, current: ProcessCpuTiming) -> Option<f32> {
    ...
    if elapsed.is_zero() || current.logical_processors == 0 || current.total_cpu_100ns < previous.total_cpu_100ns {
        return None;
    }
...
fn update_process_cpu(previous: &mut Option<ProcessCpuTiming>, current: Option<ProcessCpuTiming>) -> Option<f32> {
    let current = current?;
...
impl RuntimeSampler for PlatformRuntimeSampler {
    fn sample(&mut self) -> RuntimeSample {
        use windows::Win32::{Foundation::FILETIME, System::{ProcessStatus::{GetProcessMemoryInfo, PROCESS_MEMORY_COUNTERS_EX}, Threading::{GetCurrentProcess, GetProcessHandleCount, GetProcessTimes}}};
        ...
        let mut creation = FILETIME::default(); let mut exit = FILETIME::default(); let mut kernel = FILETIME::default(); let mut user = FILETIME::default();
        let current_cpu = unsafe { GetProcessTimes(process, &mut creation, &mut exit, &mut kernel, &mut user) }.ok().and_then(|_| {
            let to_100ns = |time: FILETIME| (u64::from(time.dwHighDateTime) << 32) | u64::from(time.dwLowDateTime);
            Some(ProcessCpuTiming { total_cpu_100ns: to_100ns(kernel).checked_add(to_100ns(user))?, observed_at: Instant::now(), logical_processors: std::thread::available_parallelism().ok()?.get() as u32 })
        });
        let process_cpu_percent = if current_cpu.is_some() { update_process_cpu(&mut self.previous_cpu, current_cpu) } else { self.previous_cpu = None; None };
        RuntimeSample { process_cpu_percent, resident_memory_bytes: memory_ok.then_some(memory.WorkingSetSize as u64), private_memory_bytes: memory_ok.then_some(memory.PrivateUsage as u64), thread_count: None, handle_count: handles_ok.then_some(handles) }
    }
}
#[cfg(not(windows))]
#[derive(Default)]
struct PlatformRuntimeSampler;
#[cfg(not(windows))]
impl RuntimeSampler for PlatformRuntimeSampler { fn sample(&mut self) -> RuntimeSample { RuntimeSample::default() } }
...
#[cfg(test)] mod tests {
    ...
    fn timing_at(start: Instant, total_cpu_100ns: u64, elapsed: Duration, logical_processors: u32) -> ProcessCpuTiming { ProcessCpuTiming { total_cpu_100ns, observed_at: start + elapsed, logical_processors } }
    #[test] fn process_cpu_first_sample_is_unavailable_until_a_prior_timing_exists() { let start = Instant::now(); let mut previous = None; assert_eq!(update_process_cpu(&mut previous, Some(timing_at(start, 0, Duration::ZERO, 4))), None); assert_eq!(update_process_cpu(&mut previous, Some(timing_at(start, 0, Duration::from_secs(1), 4))), Some(0.0)); }
    #[test] fn process_cpu_delta_is_normalized_to_total_logical_processor_capacity() { let start = Instant::now(); assert_eq!(process_cpu_percent(timing_at(start, 0, Duration::ZERO, 4), timing_at(start, 20_000_000, Duration::from_secs(1), 4)), Some(50.0)); }
    #[test] fn process_cpu_zero_delta_reports_idle_zero() { let start = Instant::now(); assert_eq!(process_cpu_percent(timing_at(start, 10, Duration::ZERO, 4), timing_at(start, 10, Duration::from_secs(1), 4)), Some(0.0)); }
    #[test] fn process_cpu_non_zero_delta_reports_usage() { let start = Instant::now(); assert_eq!(process_cpu_percent(timing_at(start, 0, Duration::ZERO, 2), timing_at(start, 5_000_000, Duration::from_secs(1), 2)), Some(25.0)); }
    #[test] fn process_cpu_invalid_timing_is_unavailable() { let start = Instant::now(); assert_eq!(process_cpu_percent(timing_at(start, 20, Duration::from_secs(1), 4), timing_at(start, 10, Duration::ZERO, 4)), None); assert_eq!(process_cpu_percent(timing_at(start, 0, Duration::ZERO, 0), timing_at(start, 1, Duration::from_secs(1), 0)), None); assert_eq!(process_cpu_percent(timing_at(start, 0, Duration::ZERO, 1), timing_at(start, 1_000_000_000, Duration::from_nanos(1), 1)), None); let mut previous = Some(timing_at(start, 0, Duration::ZERO, 1)); assert_eq!(update_process_cpu(&mut previous, None), None); assert!(previous.is_some()); }
