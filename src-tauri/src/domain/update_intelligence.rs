...
pub struct UpdateCheckResult { pub schema_version:u32, pub device_id:String, pub status:UpdateStatus, pub checked_at:Option<String>, pub support:UpdateSupport, pub package_metadata:PackageMetadata, pub updates:UpdatePackages, pub held_packages:HeldPackages, pub reboot:RebootState, pub security_updates:SecurityUpdates, pub warnings:Vec<String>, pub failure:Option<UpdateFailure> }
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)] #[serde(rename_all="camelCase")]
...
impl UpdateCheckResult { pub fn empty(device_id:String)->Self { Self { schema_version:UPDATE_SCHEMA_VERSION,device_id,status:UpdateStatus::Unknown,checked_at:None,support:UpdateSupport{status:SupportStatus::Unknown,os_id:None,os_version_id:None,package_manager:None,reason:None},package_metadata:PackageMetadata{status:MetadataStatus::Unknown,newest_metadata_at:None,age_seconds:None,stale_after_seconds:STALE_AFTER_SECONDS},updates:UpdatePackages{total_count:None,packages:vec![],truncated:false},held_packages:HeldPackages{status:HeldStatus::Unknown,total_count:None,packages:vec![],truncated:false},reboot:RebootState::Unknown,security_updates:SecurityUpdates::Unavailable,warnings:vec![],failure:None } } }
