...
- `apt-get -s upgrade` completes using the current local APT lists; and
...
| `packages` | `apt-get -s upgrade` and parse C-locale `Inst <package> [<installed>] (<candidate> ... )` records, the `The following packages have been kept back:` block, and the summary `N not upgraded` count. Normal upgrade details and separately bounded kept-back names are retained. | Required after detection. `-s` is the non-mutating APT simulation mode: it must not refresh lists or change package state. Raspberry Pi OS operator evidence showed `--no-download` can incorrectly fail this simulation with exit 100 while `-s upgrade` succeeds. A genuine non-zero exit/malformed required record is `Check failed`. |
...
UpdateCheckResult {
  ...
  keptBackPackages: { totalCount?: u32, packages: string[], truncated: boolean }
...
`checking` is ephemeral UI/action state and is never persisted. `notChecked` is represented by an absent persisted record, rather than a synthetic success. `upToDate` requires a successful required package command with both normal-update and kept-back counts zero. `updatesAvailable` requires a positive normal-update or kept-back total. Kept-back packages are deferred APT upgrade candidates, not dpkg holds; they use a separate bounded collection and must never be joined into held-package state. A successful package result with stale metadata uses `stale` as its primary status while retaining both counts and details. `unknown` is reserved for a completed result whose support cannot be determined without a classified transport/command failure; it cannot show an up-to-date claim.
...
- At most 200 update-package details, 200 kept-back names, and 200 held package names are returned and persisted. Parsing continues only until the total exceeds the bound; `truncated` is then true and the exact total count is still retained. The required APT summary count remains authoritative when kept-back names are not emitted.
